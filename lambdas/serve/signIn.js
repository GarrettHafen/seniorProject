...
exports.handler = async event => {
  ...
  if (event.httpMethod !== "POST") {
    ...
    return {
      statusCode: 405,
      body: JSON.stringify({
        error: "Method not allowed, ya dummy"
      })
    };
  ...
  if (!userCheck || !userCheck.items || userCheck.items.length !== 1 || !userCheck.items[0] || !userCheck.items[0].fields.email || userCheck.items[0].fields.email !== params.email) {
  ...
  const contenfulUser = userCheck.items[0]; // validation; check and see if the password provided matches the hash saved on the user object

  if (!(await Object(_helpers_crypto__WEBPACK_IMPORTED_MODULE_0__["compareStringToHash"])(params.password, contenfulUser.fields.password))) {
    return {
      statusCode: 400,
      body: JSON.stringify({
        error: "Invalid credentials."
  ...
  let {
    token,
    expiration
  } = Object(_helpers_crypto__WEBPACK_IMPORTED_MODULE_0__["generateToken"])(contenfulUser.fields.id);
  const userToReturn = {
